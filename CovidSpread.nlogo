...
;firstly we get following attributes state, time in the state , the getting ill sensivity, precentage of antibody , age and version
people-own [
state
time_in_this_state
...
;make a initial constructor as construc_person
to construct_person
     set shape "person"
     set state "susceptible"
     set time_in_this_state 0
     set color yellow
     set sensitivity_to_get_ill 60
     set %antibody 45
     set age random 75
...
;At the setup firstly clear the all state and reset tick
;Next we set population and set 1 infected person
;set infected person as red color
to setup
  clear-all
  reset-ticks
...
;  in the time in this state incrment and the state is dead set as dead
  ask turtles [
...
;      this condition is check random count with morality and its more than that set states
...
;      if state infected check and it multipy by recovery rate
;      if randoms check less than 100 by antibody the set state as immune and set color green
;      else set state as susceptible and color is yellow
...
;  set infected color as red and count of infected people divide by people count and multiply by 100
  set %infected (count people with [color = red] / count people) * 100
  tick
